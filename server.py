...
async def default_log_func(line):
    print(line)

async def build_aggregation(data, log_func=default_log_func, pagination=None, feed_cursor=None):
    aggregation = []

    if not data["type"] == "root":
        await log_func("Error building aggregation: no root")
        return

    if not "children" in data:
        await log_func("Error building aggregate: no root children found")
        return

    for site_node in data["children"]:
        if not "type" in site_node:
            await log_func("Error building aggregate: no type in child")
            return
        if not "filter" in site_node:
            await log_func("Error building aggregate: no filter in child")
            return
        if not "value" in site_node:
            await log_func("Error building aggregate: no value in child")
            return
        
        if not site_node["type"] == "rule":
            await log_func("Error building aggregate: root node is not a rule")
            return

        if not site_node["filter"] == "site":
            await log_func("Error building aggregate: root filter is not a site")
            return
        
        failed = False
        async def traverse(node):
            if not "type" in node:
                await log_func("Error building aggregate: node does not contain type")
                return False
            
            if node["type"] == "group":
                if not "children" in node:
                    await log_func("Error building aggregate: group node does not contain children")
                    return False

                if not "condition" in node:
                    await log_func("Error building aggregate: group node does not contain condition")
                    return False
                ...
                if not "operator" in node:
                    await log_func("Error building aggregate: rule node does not contain operator")
                    return False

                if not "value" in node:
                    await log_func("Error building aggregate: rule node does not contain value")
                    return False

                if not "filter" in node:
                    await log_func("Error building aggregate: rule node does not contain filter")
                    return False
    ...
    if pagination != None:
        if not type(pagination) is dict:
            await log_func("Error building aggregate: pagination is not a dictionary")
            return aggregation
        ...
        if not type(feed_cursor) is dict:
            await log_func("Error building aggregate: feed cursor is not a dictionary")
            return aggregation
